...
전용 branch의 code/evidence commit `4b5488d8f297638e1d8501277e6186f2b61d447b`는 Git 원격 exact HEAD `VERIFIED_R1`이었다. 이 commit 이후 변경된 54개 작은 source/test/raw-result/report 파일만 47,600-byte ZIP과 내부 manifest로 묶어 로컬 멤버 byte 검증을 통과했다. 새 Dropbox object `id:BSpOijBcT10AAAAAADt1fw`는 size와 Dropbox content hash가 맞았고, 새 Drive object `1TWRxi2W18cCaEKRss0oLXsftNy5J-_t_`는 ID/folder/size가 맞았다. 양쪽 모두 `UPLOAD_VERIFIED_R1`; 원격 raw readback과 restore는 `NOT_RUN`/`RESTORE_NOT_TESTED`다. 기존 B2 대형 배열과 과거 archive는 재업로드하지 않았다. 영수증 이후 최종 commit의 remote exact HEAD는 별도로 확인한다.
