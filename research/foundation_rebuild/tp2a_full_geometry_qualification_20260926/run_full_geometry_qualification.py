#!/usr/bin/env python3
"""Sequential-gated derivative-aware TP2A geometry qualification.

Each active geometry runs six independent operator tasks concurrently. Later
geometries are not submitted after the first failed screen. No capture, GPU,
automatic order escalation, tolerance tuning, or basis mutation exists here.
"""
...
def geometry_task_specs(z,contract,context=None):
    ...
    for method in ('phase24','reference32'):
        for dz in (-eps,0.,eps):
            row={'method':method,'z_center_a0':float(z),'dz_a0':float(dz)}
            if context is not None:row['task_id']=qr.task_identity(method,z,dz,context)
            items.append(row)
    return items
...
def main(argv=None):
    ...
    try:
        ...
        context_id=qr._digest(context);all_specs=[s for z in contract['z_samples_a0'] for s in geometry_task_specs(z,contract,context)]
        ...
        with cf.ProcessPoolExecutor(max_workers=workers,mp_context=ctx,initializer=qr.worker_init,initargs=(config,bank,native_dir,context_id,str(out))) as pool:
            def execute_geometry(z):
                specs=geometry_task_specs(z,contract,context);missing=[]
                for spec in specs:
                    try:qr.load_task(out,spec['task_id'],context_id);emit(event='task_reused',z_a0=z,method=spec['method'],dz_a0=spec['dz_a0'])
                    except FileNotFoundError:missing.append(spec)
                emit(event='geometry_start',z_a0=z,missing_tasks=len(missing),workers=workers)
                ...
                while futures:
                    ...
                    for f in done:
                        spec=futures.pop(f);res=f.result();emit(event='task_complete',z_a0=z,method=spec['method'],dz_a0=spec['dz_a0'],reused=res['reused'],wall_seconds=res['wall_seconds'])
                rows=[qr.task_row(out,s['task_id'],context_id) for s in specs]
                receipt=qr.qualify_geometry_from_tasks(z,rows,contract['epsilon_z_a0']/config['speed'],contract)
                write_json(out/'geometry'/f"z_{z:+05.1f}.json",receipt);emit(event='geometry_complete',z_a0=z,failed_screens=receipt['failed_screens'],connection_residuals=receipt['connection_residuals'],max_raw_cross_relative_difference=receipt['max_raw_cross_relative_difference'])
                return receipt
            rows,first=sequence_geometries(contract['z_samples_a0'],execute_geometry)
        report['completed_geometry']=rows
        if first:report['status']='NUMERICAL_SCREEN_FAILED';report['first_failure']=first;rc=2
        elif len(rows)!=len(contract['z_samples_a0']):raise RuntimeError('missing declared geometry results')
        else:report['status']='TP2A_DERIVATIVE_AWARE_FULL_GEOMETRY_PASS'
