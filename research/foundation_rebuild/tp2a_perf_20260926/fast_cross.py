...
def fast_cross(trajectory, channels, edges, t, *, order=24, batch=1024, progress=None, evaluator=None):
    ...
    for r0,r1,wr in _batches(edges,R,order,batch):
    ...
    return {**acc,'metadata':{'backend':('native_sp_batched_v1' if evaluator is not None else 'numpy_sp_batched_v1'),'angular':'bessel','angular_samples':n,'amplitude_degree_bound':degree,
            'radial_order':order,'radial_pairs':count,'basis_points':n*count,'batch_pairs':batch,
